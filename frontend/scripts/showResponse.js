...
const setUpClubs = async () => {
    ...
    const getteamsresponse = await fetch(model + `/footballAPI/clubfromleague/197`);
    ...
    for (let i = 0; i < club_ids.length; i++) {
        ...
        const getPlayersResponse = await fetch(model + `/footballAPI/players/${clubID}`);
        ...
        respnses.push({message : `Players fetched for club ${clubID}`, data: players});

    // Get next match
        const getNextMatchResponse = await fetch(model + `/footballAPI/nextmatch/197/${clubID}`);
        ...
        respnses.push({message : `Next match fetched for club ${clubID}`, data: nextMatch});

    //Get last match
        const getLastMatchResponse = await fetch(model + `/footballAPI/lastmatch/197/${clubID}`);
        ...
        respnses.push({message : `Last match fetched for club ${clubID}`, data: lastMatch});
    }

    renderJson({ data: { message: `Players fetched for club ${clubID}`, data: respnses}});
}
